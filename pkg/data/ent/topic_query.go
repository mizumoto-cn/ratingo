...
func (tq *TopicQuery) QueryAnalysis() *AnalysisQuery {
	query := &AnalysisQuery{config: tq.config}
...
func (tq *TopicQuery) QueryRatings() *RatingQuery {
	query := &RatingQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TopicQuery) Clone() *TopicQuery {
	...
	return &TopicQuery{
		...
		withAnalysis: tq.withAnalysis.Clone(),
...
func (tq *TopicQuery) WithAnalysis(opts ...func(*AnalysisQuery)) *TopicQuery {
	query := &AnalysisQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withAnalysis = query
	return tq
}
...
func (tq *TopicQuery) WithRatings(opts ...func(*RatingQuery)) *TopicQuery {
	query := &RatingQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TopicQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Topic, error) {
	...
	if query := tq.withAnalysis; query != nil {
		if err := tq.loadAnalysis(ctx, query, nodes, nil,
			func(n *Topic, e *Analysis) { n.Edges.Analysis = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withRatings; query != nil {
		if err := tq.loadRatings(ctx, query, nodes,
			func(n *Topic) { n.Edges.Ratings = []*Rating{} },
			func(n *Topic, e *Rating) { n.Edges.Ratings = append(n.Edges.Ratings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TopicQuery) loadAnalysis(ctx context.Context, query *AnalysisQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Analysis)) error {
	...
	query.withFKs = true
	query.Where(predicate.Analysis(func(s *sql.Selector) {
		s.Where(sql.InValues(topic.AnalysisColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.topic_analysis
		if fk == nil {
			return fmt.Errorf(`foreign-key "topic_analysis" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "topic_analysis" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (tq *TopicQuery) loadRatings(ctx context.Context, query *RatingQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Rating)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Topic)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Rating(func(s *sql.Selector) {
		s.Where(sql.InValues(topic.RatingsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.topic_ratings
		if fk == nil {
			return fmt.Errorf(`foreign-key "topic_ratings" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "topic_ratings" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
